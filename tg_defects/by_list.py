...
def make_list(source, ocr=False, seconds=4, frames=8, confidence=55):
    ...
    if ocr:
        подозрения = проверить_подряд(строки)
        if подозрения:
            log("")
            log("Проверь эти строки — номер выбивается из порядка:")
            for имя, почему in подозрения[:20]:
                log("    %-34s %s" % (имя[:34], почему))
            if len(подозрения) > 20:
                log("    ... и ещё %d" % (len(подозрения) - 20))
            log("Видео одной квартиры сняты подряд, поэтому одиночки и")
            log("разбросанные номера — первые кандидаты в ошибки.")
...
def предупредить_о_порядке(source):
    """Перед раскладкой ещё раз смотрим на список: вдруг номер выбивается."""
    path = os.path.join(source, LIST_FILE)
    if not os.path.exists(path):
        return
    try:
        with open(path, encoding="utf-8-sig", newline="") as fh:
            строки = list(csv.reader(fh, delimiter=";"))[1:]
    except OSError:
        return
    строки = [с for с in строки if len(с) >= 5 and с[1].strip()]
    подозрения = проверить_подряд(строки)
    if подозрения:
        log("Номера, выбивающиеся из порядка (проверь перед раскладкой):")
        for имя, почему in подозрения[:10]:
            log("    %-34s %s" % (имя[:34], почему))
        if len(подозрения) > 10:
            log("    ... и ещё %d" % (len(подозрения) - 10))
        log("")
...
def проверить_подряд(строки):
    """Ищет номера, выбивающиеся из порядка.

    Видео одной квартиры сняты подряд, поэтому её номер должен встречаться
    одним сплошным куском. Номер, выскочивший в одиночку между чужими или
    разбросанный по списку, — первый кандидат в ошибки распознавания.
    """
    ряд = []
    for строка in строки:
        клетка = (строка[4] or "").strip()
        цифры = "".join(c for c in клетка if c.isdigit())
        ряд.append((строка[1], int(цифры) if цифры else None))

    куски = []
    for имя, номер in ряд:
        if куски and куски[-1][0] == номер:
            куски[-1][1].append(имя)
        else:
            куски.append([номер, [имя]])

    # Одиночка между двумя кусками одной и той же квартиры — явный выброс.
    # Убираем её и сшиваем соседей, иначе они попадут в «разбросанные».
    подозрения, очищенные = [], []
    i = 0
    while i < len(куски):
        номер, файлы = куски[i]
        одиночка = (len(файлы) == 1 and номер is not None
                    and i > 0 and i + 1 < len(куски)
                    and куски[i-1][0] == куски[i+1][0]
                    and куски[i-1][0] is not None)
        if одиночка:
            подозрения.append((файлы[0],
                               "одно видео кв %d посреди кв %d"
                               % (номер, куски[i-1][0])))
            # сшиваем: следующий кусок присоединится к предыдущему
            куски[i+1][1] = очищенные[-1][1] + куски[i+1][1]
            очищенные.pop()
            i += 1
            continue
        очищенные.append([номер, файлы])
        i += 1

    сколько = {}
    for номер, файлы in очищенные:
        if номер is not None:
            сколько[номер] = сколько.get(номер, 0) + 1
    for номер, файлы in очищенные:
        if номер is not None and сколько[номер] > 1:
            подозрения.append((файлы[0],
                               "кв %d встречается в %d местах списка"
                               % (номер, сколько[номер])))
    return подозрения
