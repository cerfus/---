...
# Телефон копирует месяц словом: «2 сен. 2026 в 02:17». Цифровой записи
# там не бывает вообще, так что без этого разбора вставка с телефона
# теряет время у всех сообщений разом — а вместе с ним скорость ответов,
# инициативу и обрывы.
МЕСЯЦЫ = {"янв": 1, "фев": 2, "мар": 3, "апр": 4, "май": 5, "мая": 5,
          "июн": 6, "июл": 7, "авг": 8, "сен": 9, "окт": 10, "ноя": 11,
          "дек": 12}

_ДАТА_СЛОВОМ = re.compile(
    r"(\d{1,2})\s+([а-яё]{3,10})\.?\s+(\d{4})"        # 2 сен. 2026
    r"(?:\s*в)?\s+(\d{1,2}):(\d{2})(?::(\d{2}))?",    # в 02:17
    re.IGNORECASE)
...
def parse_date(text):
    ...
    if match:
        ...
        match = _ДАТА_СЛОВОМ.search(text)
        if not match:
            return None
        день, словом, год, час, минута, секунда = match.groups()
        месяц = МЕСЯЦЫ.get(словом[:3].lower())
        if месяц is None:
            return None            # «2 абракадабра 2026» — не дата
...
def read_pasted(текст):
    ...
    for строка in строки:
        ...
        if двоеточие and _ключ(двоеточие.group(1)) in авторы:
            if двоеточие.group(2).strip():
                _добавить(сообщения, двоеточие.group(1), None,
                          двоеточие.group(2))
                из_шапки = None
            else:
                # «Аня:» и больше ничего в строке — так копирует Телеграм
                # на телефоне: имя отдельно, текст следующей строкой. Это
                # такой же заголовок, как шапка Telegram Desktop, поэтому
                # и обрабатываем тем же способом. Раньше пустая реплика
                # отбрасывалась, а текст приклеивался к предыдущему
                # сообщению — то есть доставался не тому человеку.
                из_шапки = (двоеточие.group(1), None)
            continue
...
def безопасное_имя(имя):
    ...
    с ним уедет и всё, что мы про эту папку думаем. Расширение обязано
    уцелеть: по нему выбирается разбор.
    """
...
def прочитать_текст(путь):
    """Читает текстовый файл, подбирая кодировку.

    «Блокнот» на Windows сохраняет то в UTF-8, то в cp1251 — по настроению
    версии. Угадывать по расширению нечего, поэтому пробуем по очереди:
    UTF-8 строгой проверкой (мимо неё кириллица в cp1251 не пройдёт),
    потом cp1251, и только в крайнем случае — с заменой нечитаемого.
    """
    for кодировка in ("utf-8-sig", "cp1251"):
        try:
            with open(путь, encoding=кодировка) as fh:
                return fh.read()
