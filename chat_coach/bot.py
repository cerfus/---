...
_БЕЗ_ТЕГОВ = re.compile(r"</?\w+>")


async def _послать_разметкой(сообщение, блоки, кнопки=None):
    """Шлёт готовую разметку, складывая блоки по предел сообщения.

    Экранирования здесь нет — за безопасность отвечает тот, кто блоки
    собрал, и собирает их ровно один report.телеграмом. Зато и резать
    по символам нельзя: тег разорвётся между сообщениями, и Телеграм
    откажется принимать оба куска. Поэтому граница всегда между блоками.
    """
    куски, текущий, громоздкие = [], "", []
    for блок in блоки:
        if len(блок) > ПРЕДЕЛ_СООБЩЕНИЯ:
            # Один блок длиннее предела — резать его по границам блоков
            # нечем, а по символам нельзя. Отправим его отдельно и без
            # разметки: потерять оформление лучше, чем потерять текст,
            # потому что такое сообщение Телеграм просто отвергнет.
            громоздкие.append(блок)
            continue
        if текущий and len(текущий) + len(блок) + 2 > ПРЕДЕЛ_СООБЩЕНИЯ:
            куски.append(текущий)
            текущий = ""
        текущий = (текущий + "\n\n" + блок) if текущий else блок
    if текущий:
        куски.append(текущий)

    for номер, кусок in enumerate(куски):
        последний = номер == len(куски) - 1 and not громоздкие
        await сообщение.answer(кусок,
                               reply_markup=кнопки if последний else None)
    for номер, блок in enumerate(громоздкие):
        журнал.warning("блок отчёта длиннее предела: %d символов", len(блок))
        await _послать(сообщение, _БЕЗ_ТЕГОВ.sub("", блок),
                       кнопки=кнопки if номер == len(громоздкие) - 1 else None)
...
@диспетчер.message(Command(*texts.КОМАНДА_СТАТЫ))
async def стата(сообщение: Message):
    """Сводка по боту. Только для своего человека.

    Ничего нового не собирает: всё это пишется с первого дня — числа
    разборов, нажатия «хочу премиум», платежи, — и до сих пор не читалось
    ни разу. Пока их не видно, «что делать дальше» решается на ощупь.
    """
    if not АДМИН or сообщение.from_user.id != АДМИН:
        return          # молча: обычному человеку такой команды и не видно
    сводка = await asyncio.to_thread(база.сводка)
    await _послать(сообщение, stats.текстом(сводка), моноширинно=True)
...
async def разбор(запрос: CallbackQuery, state: FSMContext):
    ...
    недели = await asyncio.to_thread(metrics.по_неделям, сообщения, я, она)
    await _послать_разметкой(запрос.message,
                             report.телеграмом(итог, рамка, недели))
...
async def _подсказать(сообщение, state, user_id):
    ...
    if ход == metrics.ХОД_ЕЁ:
        ...
        # Про целую попытку говорит только бот: в окне тратить нечего.
        await _послать(сообщение, придержка + "\n" + texts.ПОПЫТКА_ЦЕЛА,
