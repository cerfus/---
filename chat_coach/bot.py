...
КНОПКА_ВЫГРУЗКИ = _кнопки([("Как выгрузить переписку", "как_выгрузить")])
...
async def файл(сообщение: Message, state: FSMContext):
    ...
    if not сообщения:
        await сообщение.answer(texts.ПУСТО)
        return
    if len(сообщения) < МИНИМУМ_СООБЩЕНИЙ:
        await сообщение.answer(texts.МАЛО % (len(сообщения), МИНИМУМ_СООБЩЕНИЙ))
        return
    ...
    if len(двое) < 2:
        ...
        return
    if групповой:
        await сообщение.answer(texts.ГРУППОВОЙ)

    await state.set_state(Шаги.выбор_себя)
    await state.update_data(сообщения=сообщения, двое=двое)
...
async def вопрос_без_разбора(сообщение: Message):
    await _послать(сообщение, texts.НЕТ_РАЗБОРА, кнопки=КНОПКА_ВЫГРУЗКИ)
